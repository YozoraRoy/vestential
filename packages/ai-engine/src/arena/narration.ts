...
export async function buildDiscussionSummary(params: {
  ...
  const system = [
    ...
    '差異化要求（避免每日看起來相同）：每條觀察必須引用至少一個具體標的代號或一筆具體交易（來自下方 self_reviews），並至少引用一組傳入數字（報酬率、權益、下單成功/被拒筆數）；表現最好與最差者的入選理由必須各異、不得使用相同套話。',
    '數字誠信：只可引用 ranking / self_reviews 傳入的數字，嚴禁自創、推估或四捨五入改寫報酬率與筆數；沒有的數據就明寫「未揭露」，不得腦補。',
