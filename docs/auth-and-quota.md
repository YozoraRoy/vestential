...
| **組合風險 AI 總結 (`/portfolio` 風險頁)** | 登入使用者 | 與上方共用每日 3 次（按鈕旁顯示剩餘；失敗不扣） | 台灣時間午夜 00:00 | `analysis_quota` |
| **交易日誌 AI 覆盤 (`/journal`)** | 登入使用者 | 與上方共用每日 3 次（月統計免費看，僅覆盤扣次；未滿 5 筆不給按） | 台灣時間午夜 00:00 | `analysis_quota` |
...
- **系統呼叫不佔用個人額度**：排程產生的社群文案／首回覆提問／市場總覽走系統 LLM 鏈，不扣使用者每日 3 次。
